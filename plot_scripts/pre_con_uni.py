...
for ana in analysis:
        ...
        for sche in scheduler:
            if (sche == 'distr') & (s == 'Lustre'):
                for node in nodes:
                    ...
                    markers0, caps0, bars0 = ax0.errorbar(stat.n, stat.universe, yerr=stat.universe_std, label=label, markeredgewidth=1, capsize=2, markersize=4)
                    [bar.set_alpha(0.5) for bar in bars0]
                    [cap.set_alpha(0.5) for cap in caps0]
                    markers, caps, bars = ax1.errorbar(stat.n, stat.prepare, yerr=stat.prepare_std, label=label, markeredgewidth=1, capsize=2, markersize=4)
                    [bar.set_alpha(0.5) for bar in bars]
                    [cap.set_alpha(0.5) for cap in caps]
                    markers, caps, bars = ax2.errorbar(stat.n, stat.conclude, yerr=stat.conclude_std, label=label, markeredgewidth=1, capsize=2, markersize=4)
                    [bar.set_alpha(0.5) for bar in bars]
                    [cap.set_alpha(0.5) for cap in caps]
                    tl = b[b.tags[['9000', node, ana]]]
                    stat_csv = tl.leaves().globfilter('stat*')[0]
                    stat = pd.read_csv(stat_csv.abspath)
                    markers5, caps5, bars5 = ax5.errorbar(stat.n, stat.universe, yerr=stat.universe_std, label=label, markeredgewidth=1, capsize=2, markersize=4)
                    [bar.set_alpha(0.5) for bar in bars5]
                    [cap.set_alpha(0.5) for cap in caps5]
                    markers, caps, bars = ax3.errorbar(stat.n, stat.prepare, yerr=stat.prepare_std, label=label, markeredgewidth=1, capsize=2, markersize=4)
                    [bar.set_alpha(0.5) for bar in bars]
                    [cap.set_alpha(0.5) for cap in caps]
                    markers, caps, bars = ax4.errorbar(stat.n, stat.conclude, yerr=stat.conclude_std, label=label, markeredgewidth=1, capsize=2, markersize=4)
                    [bar.set_alpha(0.5) for bar in bars]
                    [cap.set_alpha(0.5) for cap in caps]
            else:
    ...
    plt.savefig('pre_con_uni_{}.pdf'.format(ana.lower()), bbox_extra_artists=(lgd,text0, text1, text2, text5, text3, text4,))
    plt.close()
